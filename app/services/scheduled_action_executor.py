...
class ScheduledActionExecutor:
    ...
    def execute(self, scheduled_action_id):
        ...
        if scheduled_action.get("state") != "processing":
            return {
                "success": False,
                "error": "scheduled_action_not_processing",
            }

        try:
            dispatch_result = self.execute_action(scheduled_action)

            if not dispatch_result.get("success"):
                error = str(dispatch_result.get("results"))
                self._fail(scheduled_action, error)
                return {
                    "success": False,
                    "error": error,
            ...
            if not persistence_service.mark_scheduled_action_executed(
                scheduled_action_id
            ):
                return {
                    "success": False,
                    "error": "scheduled_action_finalization_failed",
                    "results": dispatch_result.get("results", []),
                }
    ...
    def build_event(self, scheduled_action):
    ...
    def _fail(self, scheduled_action, error):
        scheduled_action_id = scheduled_action["id"]
        event_id = scheduled_action.get("event_id")

        if not persistence_service.mark_scheduled_action_failed(
            scheduled_action_id,
            error,
        ):
            return
...
scheduled_action_executor = ScheduledActionExecutor()
