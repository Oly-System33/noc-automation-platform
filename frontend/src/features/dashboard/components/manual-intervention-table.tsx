  ...
  return <DashboardPanel title="Requiere intervención manual" count={props.hasResponse ? props.interventions.length : undefined} footer="Ver todas las intervenciones" isRefreshing={props.isFetching && !props.isLoading} hasError={props.isError && props.hasResponse} onRetry={props.onRetryList}>
    <div className="noc-scrollbar overflow-x-auto"><table aria-label="Requiere intervención manual" className="w-full min-w-[850px] table-fixed text-left text-[10px]">
      ...
      <tbody className="text-text-secondary">
        ...
        {!props.isLoading && !showError && !showEmpty && props.interventions.map((item) => { const pending = props.pendingId === item.interventionId; return <tr key={item.interventionId} className="h-[27px] border-b border-border-subtle last:border-b-0"><td className="px-3 whitespace-nowrap" title={item.interventionId}>{item.eventId}</td><td className="px-2 whitespace-nowrap">{item.client}</td><td className="truncate px-2 text-text-primary" title={item.failureReason}>{item.description}</td><td className="px-2 text-center"><SeverityBadge severity={item.severity} /></td><td className="px-2">{item.time}</td><td className="px-2"><div className="flex gap-2"><Button type="button" variant="outline" size="xs" disabled={!item.runbookAvailable} onClick={() => props.onViewRunbook(item)} className="h-6 min-w-[98px] rounded-[3px] border-border bg-transparent px-2 text-[9px] text-text-secondary hover:bg-surface-hover hover:text-text-primary"><BookOpen aria-hidden="true" className="size-3" />Ver runbook</Button><Button type="button" variant="outline" size="xs" disabled={pending} aria-label={item.retrySupported ? `Reintentar flujo ${item.interventionId}` : `Reintento no seguro ${item.interventionId}`} onClick={() => props.onRetryIntervention(item)} className="h-6 min-w-[112px] rounded-[3px] border-border bg-transparent px-2 text-[9px] text-text-secondary hover:bg-surface-hover hover:text-text-primary">{pending ? <RefreshCw aria-hidden="true" className="size-3 animate-spin" /> : <RefreshCw aria-hidden="true" className="size-3" />}Reintentar flujo</Button></div></td></tr> })}
      </tbody>
    </table></div>
  </DashboardPanel>
}
